...
class BasicGRU(nn.Module):
    def __init__(
        ...
        super().__init__()
        recurrent_dropout = float(dropout_rate) if int(num_layers) > 1 else 0.0
        self.gru = nn.GRU(
        ...
        self.head = nn.Sequential(
            nn.Linear(int(hidden_dim), int(mlp_dim_1)),
            nn.ReLU(),
            nn.Dropout(float(dropout_rate)),
            nn.Linear(int(mlp_dim_1), int(mlp_dim_2)),
            nn.ReLU(),
            nn.Dropout(float(dropout_rate)),
            nn.Linear(int(mlp_dim_2), 1),
        )
...
class GRUModel:
    def __init__(self, feature_name: List[str], hyper_params: Dict[str, Any], time_column: str = "Date", symbol_column: str = "Code"):
        ...
        self.model: Optional[BasicGRU] = None
    ...
    def _load_hyper_params(self) -> None:
        ...
        self.batch_size = int(self.hyper_params.get("BatchSize", 2048))
        ...
        self.hidden_dim = int(self.hyper_params.get("HiddenDim", 256))
        self.num_layers = int(self.hyper_params.get("GRUNumLayers", 1))
        self.mlp_dim_1 = int(self.hyper_params.get("MLPDim_1", 256))
        self.mlp_dim_2 = int(self.hyper_params.get("MLPDim_2", 128))
        ...
        if self.num_layers <= 0:
            raise ValueError("GRUNumLayers must be positive")

    def _build_model(self) -> BasicGRU:
        return BasicGRU(
            input_size=len(self.feature_name),
            hidden_dim=self.hidden_dim,
            num_layers=self.num_layers,
            mlp_dim_1=self.mlp_dim_1,
            mlp_dim_2=self.mlp_dim_2,
