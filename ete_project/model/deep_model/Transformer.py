...
class TransformerModel:
    ...
    def _load_hyper_params(self):
        ...
        self.embed_dim = int(self.hyper_params.get("EmbedDim", 128))
        self.num_heads = int(self.hyper_params.get("NumHeads", 4))
        self.num_layers = int(self.hyper_params.get("NumLayers", 2))
        self.ff_dim = int(self.hyper_params.get("FFDim_1", self.hyper_params.get("FFDim", 256)))
        self.head_dim = int(self.hyper_params.get("FFDim_2", self.embed_dim))
        self.train_batch_rows = int(self.hyper_params.get("TrainBatchRows", 4096))
    ...
    def _build_model(self, seq_len: int):
    ...
    def _predict_tensor(self, x: torch.Tensor, batch_size: int) -> np.ndarray:
        ...
        if x.ndim != 3:
            raise ValueError(f"Transformer input must have shape [N, L, F], got {tuple(x.shape)}")
        if self.model is None:
            self._build_model(int(x.shape[1]))
    ...
    def train(self, train_data: Any, valid_data: Any, best_model_path: str = "best_model.pth"):
        ...
        if train_data.x.ndim != 3:
            raise ValueError(f"train_data.x must have shape [N, L, F], got {tuple(train_data.x.shape)}")

        self._build_model(int(train_data.x.shape[1]))
