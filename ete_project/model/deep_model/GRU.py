...
class BasicGRU(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int, num_layers: int, head_dim: int, dropout: float):
        super().__init__()
        self.gru = nn.GRU(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        _, h = self.gru(x)
        last_hidden = h[-1]
        return self.head(last_hidden).view(-1)
...
class GRUModel:
    ...
    def _load_hyper_params(self):
        ...
        self.hidden_dim = int(self.hyper_params.get("HiddenDim", 128))
        self.num_layers = int(self.hyper_params.get("GRUNumLayers", self.hyper_params.get("NumLayers", 1)))
        self.head_dim = int(self.hyper_params.get("MLPDim_1", self.hyper_params.get("HeadDim", 128)))
    ...
    def _predict_tensor(self, x: torch.Tensor, batch_size: int) -> np.ndarray:
        ...
        if x.ndim != 3:
            raise ValueError(f"GRU input must have shape [N, L, F], got {tuple(x.shape)}")
