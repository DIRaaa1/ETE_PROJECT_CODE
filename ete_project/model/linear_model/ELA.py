...
class ELAModel:
    ...
    def _load_hyper_params(self):
        self.lr = float(self.hyper_params.get("LR", 1e-3))
        self.num_epochs = int(self.hyper_params.get("NumEpochs", 200))
        self.early_stop_patience = int(self.hyper_params.get("EarlyStopPatience", 20))
        self.alpha = float(self.hyper_params.get("Alpha", 1e-4))
        self.l1_ratio = float(self.hyper_params.get("L1Ratio", 0.5))
        ...
        if not 0.0 <= self.l1_ratio <= 1.0:
            raise ValueError("L1Ratio must be in [0, 1]")
        if self.alpha < 0:
    ...
    def _build_model(self):
        self.model = BasicELA(input_dim=len(self.feature_name)).to(self.device)
        self.optimizer = torch.optim.AdamW(self.model.parameters(), lr=self.lr, weight_decay=0.0)
    ...
    def _elastic_penalty(self) -> torch.Tensor:
        ...
        l1 = torch.sum(torch.abs(weight))
        l2 = torch.sum(weight ** 2)
        return self.alpha * (self.l1_ratio * l1 + 0.5 * (1.0 - self.l1_ratio) * l2)
    ...
    def train(self, train_data: Any, valid_data: Any, best_model_path: str = "best_model.pth"):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for xb, yb in loader:
                ...
                pred = self.model(xb)
                loss = self.criterion(pred, yb) + self._elastic_penalty()
                loss.backward()
                self.optimizer.step()
